            ...
            for task in amazon_tasks: # pylint: disable=too-many-nested-blocks
                ...
                if options.get('force', False) or ('item_count' in metadata) is False:
                    ...
                    if task.slug == 'upload-amazon-start':
                        amazon_items = query.filter(generator_identifier='webmunk-amazon-order', source=task.enrollment.assigned_identifier, created__lte=task.enrollment.enrolled)
                        item_count = amazon_items.count()
                    else:
                        amazon_items = query.filter(generator_identifier='webmunk-amazon-order', source=task.enrollment.assigned_identifier, created__gte=task.enrollment.enrolled).order_by('-recorded')
                        item_count = amazon_items.count()

                    if item_count > 0:
                        metadata['item_count'] = item_count
                        metadata['summary'] = '%d orders(s)' % item_count
